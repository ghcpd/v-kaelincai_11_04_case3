"""
Test suite for fixed authentication system
These tests validate that all bugs have been corrected
"""

import pytest
import json
import time
from fixed_auth import process_login, authenticate_user
...
def test_invalid_password():
    """Test invalid password - NOW WORKS CORRECTLY"""
...
def test_sql_injection():
    """Test SQL injection attempt - NOW PROPERLY SANITIZED"""
...
def test_non_string_password():
    """Test non-string password - NOW HANDLES GRACEFULLY"""
    result = process_login({"username": "alice", "password": 12345})
    assert result["status"] == "failure"
    assert result["message"] == "Invalid credentials"

def test_missing_username():
    """Test missing username field - NOW HANDLES PROPERLY"""
...
def test_non_dict_input():
    ...
    result = process_login("invalid input")
    assert result["status"] == "failure"
    assert result["message"] == "Invalid credentials"
...
def test_timing_attack_resistance():
    ...
    for _ in range(10):
        ...
        process_login({"username": "alice", "password": "WrongPassword123"})
        times.append(time.perf_counter() - start)
    
    # Standard deviation should be minimal (constant-time)
    import statistics
    if len(times) > 1:
        std_dev = statistics.stdev(times)
        assert std_dev < 0.01  # Very low variance indicates constant-time
...
if __name__ == "__main__":
    print("=" * 60)
    print("Running Fixed Implementation Tests")
    print("=" * 60)
